...
import socket, os, subprocess, mimetypes, multiprocessing
root = os.getcwd()
...
class Httpserver:
    def __init__(self, host, port):
        ...
        while True:
            ...
            thread = multiprocessing.Process(target=self.clientCode, args=(connection, ))
            print(f'id(s) of the threads: {os.getpid()}')
            thread.start()
            thread.join()
    def clientCode(self,connection):
